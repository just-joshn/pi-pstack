...
export async function runSpawnOrchP1bUnits() {
  ...
  try {
    ...
    // --- resume argv: MUST include --continue|-c AND --session-dir ---
    ...
    assert.ok(
      mod.argvHasContinueSemantics(resumeArgs),
      "resume argv MUST include --continue or -c",
    );
    assert.ok(resumeArgs.includes("--continue") || resumeArgs.includes("-c"));
    assert.ok(
      !mod.argvIsDirOnlyResume(resumeArgs),
      "resume argv must NOT be --session-dir alone",
    );
    ...
    // Explicit fail if someone ships dir-only as "resume"
    ...
    assert.ok(
      !mod.argvHasContinueSemantics(freshArgs),
      "fresh spawn must NOT include -c/--continue",
    );
    assert.ok(!freshArgs.includes("--resume") && !freshArgs.includes("-r"));

    // --- ephemeral: no session-dir, no continue ---
    ...
    // --- index.ts surfaces sessionDir in spawn/jobs replies ---
    ...
    assert.ok(
      indexSrc.includes("--continue") || indexSrc.includes("--continue/-c"),
      "spawn docs must cite continue argv",
    );
  } finally {
...
if (import.meta.main) {
  runSpawnOrchP1bUnits()
    .then(() => {
      console.log("PASS spawn-orch-p1b-unit");
    })
    .catch((err) => {
