...
5. **Verify citations by spot-checking.** You can read the codebase and call MCP tools or local CLIs to verify citations. Do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
...
**Explicit gaps.** Things the user asked that the evidence didn't answer. Sources searched that came up empty. Sources that weren't searchable at all, such as a missing real-time team chat MCP or CLI.
...
- **Issue / ticket tracker**: {ticket IDs and keyword searches}. Or "Not searched. No matching MCP or local CLI available on this machine."
- **Long-form documents**: {page titles and search queries}. Or "Not searched. No matching MCP or local CLI available on this machine."
- **Real-time team chat**: {channels searched, date ranges, queries}. Or "Not searched. No matching MCP or local CLI available on this machine."
- **Infrastructure observability**: {dashboards, monitors, metrics, logs, traces, or incidents searched}. Or "Not searched. No matching MCP or local CLI available on this machine."
- **Error / exception tracking**: {issues, events, or releases searched}. Or "Not searched. No matching MCP or local CLI available on this machine."
- **Product analytics warehouse**: {fully-qualified tables queried, the time windows, and the numeric summaries (counts, percentiles, first/last-seen timestamps) that bore on the question}. Or "Not searched. No matching MCP or local CLI available on this machine."
