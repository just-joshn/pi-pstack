...
Use the Linear MCP. Pi has no built-in MCP. When no MCP tool is available, use the local equivalent, the `linear` CLI, or `curl` against the Linear GraphQL API with an existing `LINEAR_API_KEY`, with the same queries. Never print or log credentials. With none of these, return a gap.
