...
On Pi the budget also applies at resolution. `resolveRoleModel` appends the level to the selector it returns (`unlimited` becomes `max`, `large` `xhigh`, `medium` `high`, `small` `medium`), so a role left at `inherit-parent` still reaches the child at the configured level, and an effort token written for a role wins over the budget.
