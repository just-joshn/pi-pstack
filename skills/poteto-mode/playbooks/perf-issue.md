...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default: your fastest strong coding model, per the pstack model rule). Review the diff. Capture a post-fix trace.
