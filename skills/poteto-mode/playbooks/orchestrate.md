...
> **Resume (close-orch-p1b):** continue a prior child via `pstack_spawn` `resumeSessionDir` (path to its `--session-dir`) and/or `resumeJobId` (in-session job with recorded `sessionDir`). Child argv is `pi --mode json -p … --session-dir <resolved> --continue`/`-c` (Pi `continueRecent` — **not** dir-only create). **Honesty:** `continueRecent` is **cwd-affined** — copying `sessionDir` alone can soft-empty the continue if the child's cwd mismatches the prior run; prefer the **same cwd** on resume, or pass an explicit `--session` file path when known. Copy `sessionDir` from spawn/jobs replies into standing store / `units.tsv` for later reattach. Fail closed if the path/job is missing. Prefer default background (omit / `true`) + `pstack_jobs` drain; sync-await needs `background: false`. Every resume still pastes standing orders / a self-contained brief — no parent-history dump. Disk job ledger is out of scope this pass.
...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, resumed, and drained only through the `pstack_spawn` tool (or `pstack_swarm` / `pstack_arena` for fan-out). State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3; nested spawn uses the Pi `pstack_spawn` schema only (local cwd/worktree isolation; no Cursor-only cloud placement args)). Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Pi v1 is local-only (no `environment` arg). Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state); multi-writer fan-out via `pstack_swarm` / `pstack_arena` auto-isolates cwd. Run a unit's verifier on a different model family from its worker. For CLI/UI proof use `pstack_control_cli` (argv) / `pstack_control_ui`.
