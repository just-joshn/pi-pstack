...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code. Conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap. Queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. In Cursor, agents are spawned, resumed, and drained only through the Task tool. On Pi, spawn and resume with `Task`; set `run_in_background: true` for the rolling window, then drain them with `SubagentAwait`, per `references/pi-runtime.md`. State reads and writes go through `scripts/orch/orch.ts` at drain points, one command in and one line out. The CLI never spawns, waits, or wakes anything.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer. Each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, and spawns its own workers and verifiers. Nesting works to depth 3 (coordinator, sub-coordinator, worker) when the parent agent definition sets `allowNestedSubagents: true`. A nested Task accepts the full schema, including `environment`. Rolls up aggregates at wave boundaries. Never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window. Never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always use `environment: "cloud"` for an isolated local worktree unless the task needs the shared checkout: `control-ui` or `control-cli` runtime verification against the running dev app, reading local transcripts under Cursor's `agent-transcripts/`, simulators and local IDE state, auth that exists only here, or uncommitted state that exists only in the main checkout. Set `cloud_base_branch` to a fetched named ref. Cloud Tasks require a clean source checkout and start from a committed ref. They can read the store by absolute path, so include that path or inline the needed content. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
Create `orchestrate/<project-slug>/` in the current agent's store (path in the system prompt). In Pi, keep it under `pstack/` in Pi's agent directory (`$PI_CODING_AGENT_DIR`, default `~/.pi/agent`). It lives outside every worktree so isolated Tasks can reach it by absolute path. Create one goal for the program with `CreateGoal({objective: "<program objective>"})`. Record each returned `agent_id` in the store so you can probe the run after a restart. Every file has exactly one writer. Owners publish facts, readers aggregate at read time. Use `bun scripts/orch/orch.ts` for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. A 4KB scaffold around a two-line edit costs more to write and obey than the edit. Local spawns may reference the standing-orders file by store path. Verbatim paste is for worktree-isolated spawns and every resume.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the `environment: "cloud"` default and the `environment: "local"` exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (a background `Shell` watcher that prints a unique sentinel on the event, with a long one-shot Shell heartbeat fallback, per `references/pi-runtime.md`), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), writes the resulting rows through `orch unit add`, `orch unit set`, and `orch ledger record`, runs `orch status`, then launches the next wave with parallel `Task` calls in one message.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Restacks run in a managed local worktree lane, the Pi replacement for a cloud lane, one at a time. Parallel restacks at this scale take the laptop down.
...
A unit is not done until its output is externalized the moment it lands, never batched to the end of the run. A worker pushes its branch, a verifier writes its ledger row, receipts land in the store. Work that exists only in one worktree when that worktree is discarded was never done.

#### Liveness and failure

- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and the agent's status in the Cursor dashboard, mapped on Pi to `SubagentAwait({agent_id, timeout_ms: 0})`; use `/goal` for the goal. Transcript mtime is not liveness.
...
- After a Pi restart: background Tasks run in detached processes and may still be active. Re-read the standing orders and `units.tsv`, recompute the frontier, and probe each recorded `agent_id` with `SubagentAwait({agent_id, timeout_ms: 0})`. Reattach live work by PR and branch when an id is unavailable. Respawn one sub-coordinator per track from its stored brief plus current state, drain, and resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
