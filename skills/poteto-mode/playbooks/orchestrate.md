...
> **Resume (close-orch-p1):** continue a prior child via `pstack_spawn` `resumeSessionDir` (path to its `--session-dir`) and/or `resumeJobId` (in-session job with recorded `sessionDir`). Fail closed if the path/job is missing. Prefer default background (omit / `true`) + `pstack_jobs` drain; sync-await needs `background: false`. Every resume still pastes standing orders / a self-contained brief — no parent-history dump. Disk job ledger is out of scope this pass.
