...
- [ ] Use one `Task` per owner with `description`, `prompt`, `subagent_type: "poteto-agent"`, `run_in_background: true`, `environment: "cloud"`, and `cloud_base_branch` set to the fetched PR base.
...
- [ ] Record a 30 to 60 second video of the change in a lane's managed local worktree. Save it as `<media path>/<pr-id>-review.mp4`.
...
- [ ] When every box is checked and the predicate is true, call `UpdateGoal({status: "COMPLETE"})`.
