...
1. Locate the prior trail. A local transcript under the active workspace's `agent-transcripts/` directory (on Pi, the local equivalent is the directory holding `$PI_SESSION_FILE`, per `references/pi-runtime.md`. Do not glob across `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/sessions/*/`, that crosses workspace boundaries and reads private chats from unrelated projects), a cloud-agent run's `agent_id`, or a pushed branch. On Pi, a cloud-agent run is a worktree Task; read it with `SubagentAwait({agent_id, timeout_ms: 0})`, then its transcript and artifact paths. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
