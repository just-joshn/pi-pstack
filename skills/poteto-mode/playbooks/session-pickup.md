...
1. Locate the prior trail. A local transcript under `~/.pi/agent/sessions/--<path>--/` (`<path>` is the absolute working directory with the leading slash dropped and every remaining `/` replaced by `-`, so `/Users/you/proj` becomes `--Users-you-proj--`; do not glob across `~/.pi/agent/sessions/*/`, that crosses project boundaries and reads private chats from unrelated projects), a session file the user names, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
