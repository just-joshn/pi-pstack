...
1. Locate the prior trail. A local transcript under the active workspace's Pi session transcripts directory (the system prompt names the path. Do not glob across `~/.pi/agent/sessions/*/`, that crosses workspace boundaries and reads private chats from unrelated projects), a stored `sessionDir` or a prior `pstack_jobs` reply, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
