...
4. Verify each component against its baseline via image diff on the matching surface through the project's harness (the visual-regression setup from step 1). A nonzero diff is a fail. Investigate the pixel delta. Iterate per component until the diff is zero.
