...
**Worktree.** Work from a git worktree off main. Agents use the current branch by default. Multiple `Task` writers on the same branch each get an isolated worktree with `environment: "cloud"` and `cloud_base_branch: "origin/<branch>"`, or use `git fetch && git reset --hard origin/<branch>` between them. Dirty branch with unrelated work: patch out, fresh worktree, apply. Snarled worktree: reset from main, redo minimally.
