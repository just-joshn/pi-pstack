...
const LANES = [
	...
	"Ten lanes on the configured swarm-workers model at the PR head",
];
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.some((s) => live.rest.includes(s))) fail(live.n, `${pr.title}: Verify, live lacks "${LANES[0]}"`);
	...
	if (gate) {
		...
		if (gate.rest.startsWith("None.")) {
			...
			const words = ["screenshot", "video", "operator"];
			if (!gateBoxes.some((b) => words.every((w) => b.text.includes(w)))) {
				fail(gate.n, `${pr.title}: no Review gate box contains "screenshot", "video", and "operator"`);
			}
		}
	}
