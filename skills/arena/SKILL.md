...
Call `pstack_arena` (preferred) or N× `pstack_spawn` with unique `cwd`s. `background` on spawn is sync-awaited in v1 — use `pstack_arena` for parallel fan-out (global concurrency cap: 4). Each candidate gets the task, the path to the shared grounding, its own output path, and instructions to produce both the artifact and a short rationale.
