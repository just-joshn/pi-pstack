...
1. Pick a kebab-case slug for the routine. Use it for the directory under Pi's agent directory (`$PI_CODING_AGENT_DIR`, default `~/.pi/agent`): `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/pstack/bots/<slug>/`.
...
3. Pick the session: `--session-dir $BOT/sessions --session-id <slug>`, run with cwd `$BOT` (`${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/pstack/bots/<slug>/`). `--session-id` creates the session on the first wake, and every later wake appends to it, so the bot keeps its memory across wakes.
4. Pick the reply channel. The bot's final text goes to `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/pstack/bots/<slug>/replies.log`, and the page may show the newest entry. Use a desktop notification (`osascript -e 'display notification ...'` on macOS, `notify-send` on Linux) only if the user asks.
