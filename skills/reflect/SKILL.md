...
Each reviewer and the synthesizer name a role line in the `pstack-models.mdc` rule (on Pi, the pstack block) and a default. On Pi, that block is in `${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/AGENTS.md`. Set `model` to that line's value, or to the default if the block or line is missing. Omit `model` when the value is `auto` or `inherit-parent`. If Task rejects a configured model, use the default and say so. If Task rejects the default, use the closest valid model of the same provider family from its error message.
