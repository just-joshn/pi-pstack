...
- `read` tool calls against any `SKILL.md` file (project `.pi/skills/` or `.agents/skills/`, user-level `$PI_CODING_AGENT_DIR/skills/` in Pi's agent directory (`$PI_CODING_AGENT_DIR`, default `~/.pi/agent`), or `~/.agents/skills/`, or Pi package paths)
