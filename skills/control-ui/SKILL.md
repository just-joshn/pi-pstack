...
- Creating before/after evidence for a pstack playbook, or for the project's `verify-<app>` skill (see the **create-verification-skill** skill).

## Setup Pattern

1. Start the app locally using the repo's documented dev command. Pi's `bash` tool waits for commands to exit, so run a long-lived dev server detached (`tmux new-session -d -s ui-<slug> '<dev command> 2>&1 | tee /tmp/ui-<slug>.log'`, or `nohup <dev command> > /tmp/ui-<slug>.log 2>&1 &`), then poll the log or the port for readiness.
...
When several pstack verification lanes (swarm workers, arena candidates, live lanes) drive UIs on this machine at once, give each lane its own worktree, dev-server port, debug port, and browser profile directory (`--user-data-dir=/tmp/ui-<slug>-<n>`). Name artifacts per lane, for example `/tmp/swarm-<pr-id>/worker-<n>/<slug>.png`. Lanes never share a browser or a server.
