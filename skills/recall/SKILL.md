...
Transcripts live at `~/.pi/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl`, where `<path>` is the absolute working directory with the leading slash dropped and every remaining `/` replaced by `-` (so `/Users/you/proj` becomes `--Users-you-proj--`). Every line is one session entry; chat messages are entries with a `message` field.
