...
Transcripts live at `$PI_CODING_AGENT_DIR/sessions/--<slug>--/<timestamp>_<id>.jsonl` in Pi's agent directory (`$PI_CODING_AGENT_DIR`, default `~/.pi/agent`), where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`, and the directory is `--Users-you-proj--`). The current chat is `$PI_SESSION_FILE`. Task sessions are `<timestamp>_<id>/<run id>/run-<n>/session.jsonl`. Every line is one JSON event. Chat turns are the `{"type":"message"}` lines, with `message.role` and `message.content`. These transcript paths follow Pi's runtime in `../poteto-mode/references/pi-runtime.md`. Loading this skill authorizes the Task calls it prescribes.
