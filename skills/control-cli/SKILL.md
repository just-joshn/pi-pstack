...
3. If no harness exists, launch the CLI in an isolated terminal session with deterministic env vars. Pi's `bash` tool has no TTY and waits for each command to exit, so never run an interactive CLI directly in it. Drive it through tmux or a PTY probe.
...
When several pstack verification lanes drive CLIs on this machine at once, give each lane a unique tmux session name, its own worktree, its own temp `HOME` or config directory when the CLI writes state, and its own artifact directory. Lanes never share a session.
