...
test("pinned upstream tree is resolvable", () => {
  expect(upstream, "no pinned upstream clone found; run `npm run parity:check` to populate .port-upstream/cursor-plugins-<sha7> or set PORT_UPSTREAM_DIR").toBeTruthy();
});
...
test("every in-scope upstream artifact is covered by a parity row", () => {
  ...
  expect(uncovered.length === 0, `uncovered upstream artifacts:\n${uncovered.map((rel) => `  ${rel}`).join("\n")}`).toBeTruthy();
});
...
test("every row pointing at an upstream path names a file in the tree", () => {
  ...
  expect(absent.length === 0, `rows point at absent upstream paths:\n${absent.map((line) => `  ${line}`).join("\n")}`).toBeTruthy();
});
