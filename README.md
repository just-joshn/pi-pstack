...
**pstack is my answer.** these are the same skills i use everyday to ship high quality code at Cursor. this turns your coding agent into a real engineering team. the goal is not to maximize loc, in fact it's the opposite. pstack helps you write less, but higher quality code.
...
or straight from git (or npm, once published):

```bash
pi install git:github.com/you/pstack@main
```

then run `pi` in any project. the package ships its skills, its extensions, and its subagents.
...
that's it. the other skills are situational; the mode skill uses them for you as needed. out of the box the mode splits work by model strength: code delegates (feature, refactoring, bug fix, perf, hillclimb) go to your fastest strong coding model, while the hardest changes, prose, and judgment go to your strongest reasoning model. the default review panel is one strong model per family you have access to. [`/setup-pstack`](./skills/setup-pstack/SKILL.md) changes any of it.
...
every skill is a slash command, exactly like the cursor plugin: `/poteto-mode`, `/how`, `/why`, `/tdd`, `/interrogate`. pi's native spelling (`/skill:poteto-mode`) works too.
...
overnight runs stay alive through wake mechanisms: watcher subagents, heartbeats, or a shell loop that re-prompts `pi` on an interval. you can keep it working for many hours without sacrificing rigor.
...
pstack also ships a subagent that runs my style end to end. spawn it from a parent agent through the `subagent` tool with `agent: "poteto-agent"` ([definition](./agents/poteto-agent.md)). it reads `poteto-mode` in full, including its inline principles index, before doing any work. substituting the `worker` agent skips that read and drifts.

[`/poteto-mode`](./skills/poteto-mode/SKILL.md) and [`poteto-agent`](./agents/poteto-agent.md) route through the same wrapper.

pstack also ships [Comment Sicko](./agents/comment-sicko.md), a read-only comment reviewer available as `agent: "Comment Sicko"` on the subagent tool. usually invoke it through [`/no-comments`](./skills/no-comments/SKILL.md), not directly.

two more bundled agents round out the fleet: `reviewer` (read-only analysis, used by interrogate panels, arena judges, and how explorers) and `worker` (general-purpose delegate).
...
this is a port of the cursor plugin to [pi](https://pi.dev). the pieces that make it behave the same:

- **skills** live in `skills/` in the standard agent-skills format. pi loads them from the package. every skill keeps `disable-model-invocation: true`, so nothing fires behind your back: you invoke exactly what you asked for.
- **slash commands.** in cursor every skill is a slash command. the `pstack-slash` extension accepts the same spelling here, args included. `/skill:poteto-mode` works too.
- **subagents.** the bundled `subagent` extension is pi's official subagent example, extended: single, parallel, and chain modes; per-task `model` overrides (`provider/model-id`, optionally `provider/model-id:thinking`) for the multi-model skills; and the bundled agents above.
- **todolists.** the bundled `todo` extension registers the `todo` tool and a `/todos` command, so playbooks can open a todolist and you can watch it.

## not shipped here

a few things `poteto-mode` references but doesn't bundle:

- the commit slop pass (`/deslop` in cursor's `cursor-team-kit`) is inlined into `poteto-mode` as a rule: the unslop discipline applied to code before each commit.
- the control skills (`control-cli` and `control-ui`) don't exist here. drive the real surface through your project's harness: a `verify-*` skill, Playwright, or the app's own scripting.
- pi doesn't ship a `/create-skill` or a built-in `/babysit`. authoring follows the [authoring-a-skill playbook](./skills/poteto-mode/playbooks/authoring-a-skill.md), and pr-status requests always route to the [babysit playbook](./skills/poteto-mode/playbooks/babysit.md).

## why are there no planning skills?

pi doesn't ship a plan mode, and personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/poteto-mode`](./skills/poteto-mode/SKILL.md) covers it (the multi-phase plan playbook), but it's not a default. 
...
models are configurable too. type [`/setup-pstack`](./skills/setup-pstack/SKILL.md). it detects the models you have access to and writes a small always-applied block in `~/.pi/agent/AGENTS.md` mapping each role (code, judgment, the review panels) to a model. every skill reads it and falls back to sensible defaults when the block is absent, so you override only what you want.
...
to set it up, point the agent at [`FOR_AGENTS.md`](./automations/benny/FOR_AGENTS.md). setup copies the pack into the target repository at `.pi/automations/benny/`, enables pstack there for shared skills, and keeps user configuration outside the copied pack. the two runs are scheduled with cron (or any scheduler) piping each template prompt into `pi -p` in the target repository.
