...
function createRuntime(options) {
  ...
  return { now, leaseMs, token, store, execute, inFlight: new Map() };
}
...
function startExecution(runtime, record, envelope) {
  ...
  runtime.inFlight.set(runId, controller);
  const context = {
    runId,
    attempt,
    envelope,
...
async function handlePostTask(runtime, req, res) {
  ...
  const validated = validateTaskRequest(parsed);
...
export function main() {
  ...
  const server = createWorkerServer({ token });
  if (!token) {
