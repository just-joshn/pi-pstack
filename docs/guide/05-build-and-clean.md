...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs a slop pass over the diff before each commit (the unslop discipline applied to code) and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. There is no separate code-slop skill here, so the playbook states the outcome directly: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits. Ask for that in plain words any time you want it on demand.
...
The division of labor is worth keeping straight. The commit slop pass cleans slop out of the code, `/unslop` cleans it out of prose, and `/no-comments` hands the comments to a reviewer who didn't write them.
