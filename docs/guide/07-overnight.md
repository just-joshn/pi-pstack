...
- A wake mechanism (watchers, heartbeats, a shell loop that re-prompts `pi`) is what keeps an overnight run alive. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
