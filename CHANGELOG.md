...
### Added
- `tests/inventory/skills-doc-conformance.test.mjs` pins Pi skills-documentation conformance for every shipped SKILL.md in `npm test`: the real `loadSkills` loader reports zero diagnostics, names and descriptions meet the Agent Skills limits, each name matches its directory, bodies stay under the 500-line progressive-disclosure budget, every relative reference resolves from the skill root, and no skill file hardcodes an absolute local path.
...
### Fixed
- Fifteen skill references now resolve from the skill root the Pi system prompt names. Six nested reference docs under `skills/` and `automations/benny/skills/` pointed at their targets file-relative, so the new `port/bindings/rules-links.mjs` binding group rewrites the `why` source links, the `why` investigator template, the `architect` rationale and runner links, the verification-skill feature-map example, and the benny control adapter. The change stays inside the declared bindings, so `npm run parity:check` holds content parity with upstream.
