...
> The live contract and reference status now live in [`spec/SPEC.md`](./spec/SPEC.md) and [`spec/contracts/`](./spec/contracts/). This file is the historical snapshot for the v0.15.2 port; its status rows are not the live authority. Run `npm run spec:check` for current coverage.
...
## Behavioral scorecard (12 capabilities, historical snapshot)
...
| 5 | shipping / babysit (gh) | **EQUIVALENT** (local-gh) | **Local-scope criteria:** `pstack_babysit` + `pstack_ship` with fail-closed `evaluateMergeGates` (fixture matrix ≥8 cases); babysit defaults to concrete `watchArgv` recipe + `pstack_loop mode=dynamic` arm payload; gate-check before merge. Not Origin-first / cloud-verifier luxuries. Correction: evaluateMergeGates is deliberately stricter than the ported watch-pr policy, blocking BLOCKED, BEHIND, REVIEW_REQUIRED, and pending checks; this divergence is recorded in spec/SPEC.md section 4. |
| 6 | deslop / control companions | PARTIAL | `applySafe` + **`dryRun`** + expanded pattern set; optional `autoApply` via `ui.confirm`. **Residuals (≤3):** (1) pattern depth still thinner than cursor-team-kit `/deslop`; (2) `pstack_control_ui` is HTTP probe only (no Electron/IDE drive); (3) prose cleanup still pairs with `/skill:unslop` rather than a full team-kit workflow. Correction: the residual comparison baseline cursor-team-kit has no in-tree spec here; the owned pieces unslop and no-comments are byte-identical or binding-only. |
...
| 8 | recall | **EQUIVALENT** (local-recall) | **Local-scope criteria:** `pstack_sessions` action=`recall` rebuilds topic context via **ranked merge** of Pi sessions + `git log` + `gh` PRs (when available) without Cursor transcripts. Completes local “rebuild topic context” workflow under Pi corpus. Correction: only the local corpus (sessions plus git plus gh PRs) exists; shared-record MCP sources are unreachable and are recorded as a host ceiling. |
...
| 11 | Ask-mode / readonly semantics | **EQUIVALENT** (local-readonly) | **Local-scope criteria:** `/pstack-readonly` strips write/edit/bash; sticky investigation playbook **auto-arms** readonly; spawn `investigator`/`comment-sicko` auto-readonly allowlist; `tool_call` blocks writes (and mutating ship/worktree/deslop-apply). **Not** Cursor Ask-mode MCP-strip — scored as local-readonly only. The `tool_call` policy table covers every registered pstack tool, including `pstack_loop`/`pstack_run` (inspection allowed, arm blocked), `pstack_decision_log` (blocked), and `pstack_benny_wake` (path allowed, write blocked), in `extensions/readonly-state/index.ts`; a census test fails when a registered tool has no policy. |
| 12 | Automations / cloud agents / marketplace | NOT | Local spawn+worktree twin only; marketplace/Automations/cloud VMs absent. Correction: a definable local or remote worker path exists for the cloud half; only the marketplace and Automations editor are true exceptions. |
