...
| # | Capability | Behavioral | Notes (Stage 2 close-orch-p0) |
|---|---|---|---|
...
| 2 | Task / subagent → spawn / swarm / arena | **EQUIVALENT** (local-Task) | **Local-scope IN:** `inheritParentTools` → child allowlist from `getActiveTools()` (readonly roles force `READONLY_TOOLS`); `pstack_jobs` list/status/await/abort|cancel within session; **MAX_CONCURRENCY** default **8** (shared spawn/swarm/arena; env `PSTACK_MAX_CONCURRENCY`); poteto skill prefers `background: true` + `pstack_jobs` drain; `sessionMode=isolated`; `persistOutput` on for bg/long; clean-context children (self-contained briefs + file pointers). **Certified ceilings (not blocking):** Pi has no first-party MCP — Cursor-local MCP inherit **N/A** (host ceiling); parent transcript inherit **N/A** — Cursor Subagents also start clean; session-scoped jobs (die on `session_shutdown`) = Cursor-local restart behavior — **not** a durable-daemon gap. Resume/`resumeSessionDir` is P1 follow-on for Orchestrate fidelity, not a row-2 gate. |
...
**Verifier-aligned scorecard (close-orch-p0):** **EQUIVALENT** (local-scope): **1** sticky, **2** spawn/Task (local-Task), **3** loop, **4** worktrees, **5** shipping/babysit, **7** models, **8** recall, **11** readonly. **PARTIAL:** **6** deslop (≤3 residual bullets). Out-of-scope **NOT:** 9, 10, 12. Never claim Cursor sticky-host / Ask-MCP / native `/loop` chrome / first-party MCP bit-identical.
