...
**Verifier-aligned scorecard (close-local-v3 + sticky fix):** **EQUIVALENT** (local-scope): **1** sticky (after restore reinject + non-silent force-invoke), **3** loop, **4** worktrees, **5** shipping/babysit, **7** models, **8** recall, **11** readonly. **PARTIAL:** **2** spawn/Task, **6** deslop (each ≤3 residual bullets). Out-of-scope **NOT:** 9, 10, 12. Never claim Cursor sticky-host / Ask-MCP / native `/loop` chrome bit-identical.
